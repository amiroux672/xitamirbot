...
async function onCall({ message, getLang, args }) {
  ...
  if (message.body && ("chẵn" == message.body.toLowerCase() || "lẻ" == message.body.toLowerCase())) {
    ...
    if (1 == (i = bcl.player[c]).choose.status) return global.api.sendMessage("بمجرد اختيارك ، لا يمكنك الاختيار مرة أخرى!", message.threadID, message.messageID);
    ...
    }), global.api.sendMessage(`${i.name} تم اختياره بغرابة`, message.threadID, message.messageID));
    var m = 0,
      u = bcl.player.length;
