const config = {
  name: "تيكتوك",
  description: "info account ttok",
  usage: ["توك","تيك","معلومات_حساب"],
  credits: "AMIROUX",
  cooldown: 5,
};
//dân ửa lồn tuổi với anh kkk
//địt mẹ vua móc lồn 
const langData = {
  ar_AR: {
    result:
      "id tik tok: {id}\nالاسم: {user}\nالصورة الرمزية (Cỡ lớn): {avatarthumb}\nAvatar (Cỡ trung bình): {avatarmedium}\nالصورة الرمزية (Cỡ lớn): {avatarlarger}\nتحقق: {verified}\nالتالي: {dangfollow}\nمتابعون: {follower}\nقناة يوتيوب: {kenhyt}\nتويتر: {twitter}\nانستغرام: {instagram}\nضرب القلب: {heart}\nفيديو: {video}",
    missingInput: "الرجاء إدخال معرف حساب الفيسبوك الخاص بك",
    notFound: "لاتوجد بيانات.",
    error: "خطأ! حدث خطأ. اذا سمحت حاول مرة أخرى لاحقا. نأسف للإزعاج.",
  }
};
async function onCall({ message, args, getLang }) {
  try { 
    const input = args[0]; 
    if (!input) return message.reply(getLang("missingInput"));
    const encodedInput = encodeURIComponent(input);
    const url = `https://tiktok--hehehehehehehehehhehehee.repl.co/?user=${encodedInput}`;
    const res = await global.GET(url);
    const data = res?.data || {};

    if (Object.keys(data).length === 0) {
      return message.reply(getLang("notFound"));
    }
     const verified = res.data.tichxanh ? "Đã được xác minh" : "Không được xác minh"
    const response = getLang("result", {     
      id: data.id,
      user: data.user,
      nickname: data.nickname,
      avatarthumb: data.avatarthumb,
      avatarmedium: data.avatarmedium,
      avatarlarger: data.avatarlarger,
      signature: data.signature,
      verified: verified,
      dangfollow: data.dangfollow,
      follower: data.follower,
      kenhyt: data.kenhyt,
      twitter: data.twitter,
      instagram: data.instagram,
      heart: data.heart,
      video: data.video,
    });
    return message.reply(response);
  } catch (e) {
    console.error(e);
    return message.reply(getLang("error"));
  }
}
export default {
  config,
  langData,
  onCall,
};